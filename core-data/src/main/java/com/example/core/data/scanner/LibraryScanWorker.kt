...
class LibraryScanWorker @AssistedInject constructor(
    ...
    companion object {
        ...
        const val KEY_FOLDER_ID = "folder_id" // ID папки для группировки файлов
    ...
    override suspend fun doWork(): Result {
        try {
            ...
            // Получаем folderId из входных данных (если папка была выбрана пользователем)
            val folderId = inputData.getString(KEY_FOLDER_ID)
            if (folderId != null) {
                android.util.Log.d(TAG, "Scanning with folder ID: $folderId")
            }
            ...
            // ВАЖНО: Передаем folderId только если он был указан при сканировании папки
            // Файлы, добавленные по отдельности, не должны получать folderId автоматически
            var addedCount = 0
            for (file in comicFiles) {
                try {
                    // Передаем folderId только если он был указан (сканирование папки)
                    // Если folderId == null, файл не будет привязан к папке (добавлен по отдельности)
                    val comic = metadataExtractor.extractMetadata(file, folderId)
                    if (comic != null) {
                        ...
                        if (existing == null) {
                            ...
                            addedCount++
                            android.util.Log.d(TAG, "✅ Added comic: ${comic.title}, folderId: ${comic.folderId ?: "null"}")
                        } else {
                            android.util.Log.d(TAG, "⚠️ Comic already exists: ${file.name}")
                        }
                    }
                } catch (e: Exception) {
