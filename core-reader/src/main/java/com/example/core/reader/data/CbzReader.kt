...
class CbzReader @Inject constructor(
    ...
    override suspend fun renderPage(
        ...
        return@withContext try {
            ...
            val imageData = pageByteCache.get(rawCacheKey) ?: run {
                ...
                android.util.Log.d("CbzReader", "Extracted page $pageIndex bytes=${bytes.size}")
                if (bytes.isNotEmpty()) {
            ...
            while (bitmap == null && retryCount < maxRetries) {
                try {
                    bitmap = BitmapUtils.decodeBitmapWithExif(
                        imageData,
                        maxWidth,
                        maxHeight,
