...
## S2 + S4: ARC-11 chrome sheet host + key action policy

- **Срез S2 выполнен (`arc-11-bottom-sheet-host 2026-08-09`)**: `ReaderBottomSheets` раньше принимал 35+ параметров в call-site (9 read-only + 11 (state, setter)-пар + 4 launchers) — самый длинный call-site в `ReaderScreen()`. После выноса: новый internal-data-class `ReaderBottomSheetHost` собирает все 35 полей, `@Composable rememberReaderBottomSheetHost(...)` — Compose-обёртка с `remember{}` по всем ключам, `ReaderBottomSheets(host: ReaderBottomSheetHost)` принимает один аргумент, тело ссылается через `host.fieldName`. `ReaderScreen.kt` call-site свёрнут до одного `host = rememberReaderBottomSheetHost(...)`. **7 unit-тестов** в `ReaderBottomSheetHostTest` (plain JUnit + mockk для тяжёлых типов): data-class equals/copy/toString контракт, что setter-лямбды реактивны через `host.copy()` (так их использует [ReaderScreen]), и reflection-тест, что все поля остаются `val` (фиксирует контракт против случайного превращения в `var`). Equality-тест через mockk-with-relaxed невозможен — но это особенность mockk, не нашего data-class.
- **Срез S4 выполнен (`arc-11-key-action-policy 2026-08-09`)**: `readerVolumePagingStep` + `resolveReaderHardwareKeyDecision` + `ReaderHardwareKeyDecision` с memoed в `ReaderInteractionPolicy.kt` вместе с другими interaction helpers. После выноса: `ReaderInteractionPolicy.kt` 217 → 179 строк (-38); новый `ReaderKeyActionPolicy.kt` (95 строк) ровно про клавиатуру. Pure-primitive signature `resolveReaderHardwareKeyDecision(keyCode, action, repeatCount, ...)` без `android.view.KeyEvent`-dependency — для тестов без Robolectric и `KeyEvent`-моков; convenience `KeyEvent`-overload сохранён для Compose-кода в [ReaderScreen.kt]. **10 plain-JUnit тестов** в `ReaderKeyActionPolicyTest`: volume-up/down steps для LTR/RTL/WEBTOON/DUAL_PAGE; non-volume клавиши (BACK/MENU/DPAD/PAGE_UP/SEARCH) → null; первое ACTION_DOWN отдаёт pageStep; held key (repeatCount>0) проглатывается без повторного pageTurn; ACTION_UP consumed; volumePagingDisabled проглатывает всё (a11y/TTS-volume); ACTION_MULTIPLE не наш. Старый volumePaging-тест из `ReaderInteractionPolicyTest.kt` удалён как дубликат.
