...
- Остаток 4.1: ревизия оставшихся extension-файлов Settings (`SettingsViewModelAppearanceSetters`/`ReaderSetters`/`TextSetters`/`GoalsSetters`/`LibrarySetters`/`PerformanceSetters`/`TranslationSetters` — кандидаты в контроллеры-сеттеры); Library — аудит `LibraryViewModelDisplay`/`Helpers` на статeless-вынос.
