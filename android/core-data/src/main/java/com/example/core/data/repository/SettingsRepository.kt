...
interface SettingsRepository {
    ...
    suspend fun restoreLocalBackup(backupFileName: String)
    ...
    val enableDualPageMode: Flow<Boolean>
    suspend fun setEnableDualPageMode(enabled: Boolean)
    val enableWebtoonMode: Flow<Boolean>
    suspend fun setEnableWebtoonMode(enabled: Boolean)
...
class SettingsRepositoryImpl @Inject constructor(
    ...
    // Используем Provider для разрыва циклической зависимости: SettingsRepository <-> LibraryBackupManager
    private val libraryBackupManagerProvider: javax.inject.Provider<LibraryBackupManager>
) : SettingsRepository {
    ...
    private object PreferencesKeys {
        ...
        val ENABLE_DUAL_PAGE_MODE = stringPreferencesKey("enable_dual_page_mode")
        val ENABLE_WEBTOON_MODE = stringPreferencesKey("enable_webtoon_mode")
    }
    ...
    override val zoomAnimationDuration: Flow<Long> = dataStore.data.map {
        it[PreferencesKeys.ZOOM_ANIMATION_DURATION]?.toLongOrNull() ?: 200L // 200ms по умолчанию
    }
    ...
    override val brightnessAnimationDuration: Flow<Long> = dataStore.data.map {
        it[PreferencesKeys.BRIGHTNESS_ANIMATION_DURATION]?.toLongOrNull() ?: 300L // 300ms по умолчанию
    }
    ...
    override val resetZoomOnPageChange: Flow<Boolean> = dataStore.data.map {
        it[PreferencesKeys.RESET_ZOOM_ON_PAGE_CHANGE]?.toBoolean() ?: true // true по умолчанию
    }
    ...
    override val imageRenderDpi: Flow<Int> = dataStore.data.map {
        it[PreferencesKeys.IMAGE_RENDER_DPI]?.toInt() ?: 3200 // Увеличено для лучшего качества
    }
    ...
    override suspend fun createLocalBackup() {
        try {
            // Создаём бэкап в Downloads/MrComicBackups
            val downloadsDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
            val backupDir = File(downloadsDir, "MrComicBackups")
            if (!backupDir.exists()) {
                backupDir.mkdirs()
            }

            val timestamp = System.currentTimeMillis()
            val backupFile = File(backupDir, "mrcomic_backup_$timestamp.mrcomic")

            val backupUri = Uri.fromFile(backupFile)

            libraryBackupManagerProvider.get().exportBackup(backupUri).fold(
                onSuccess = { summary ->
                    updateLastBackup(backupUri.toString(), timestamp)
                    android.util.Log.d("SettingsRepository", "✅ Local backup created: ${backupFile.absolutePath}, comics: ${summary.comicsCount}, bookmarks: ${summary.bookmarksCount}")
                },
                onFailure = { error ->
                    android.util.Log.e("SettingsRepository", "❌ Failed to create local backup", error)
                    throw error
                }
            )
        } catch (e: Exception) {
    ...
    override suspend fun restoreLocalBackup(backupFileName: String) {
        try {
            val downloadsDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
            val backupDir = File(downloadsDir, "MrComicBackups")
            ...
            if (!backupFile.exists()) {
                android.util.Log.w("SettingsRepository", "⚠️ Backup file not found: ${backupFile.absolutePath}")
                throw IllegalArgumentException("Backup file not found: $backupFileName")
            }

            val backupUri = Uri.fromFile(backupFile)

            libraryBackupManagerProvider.get().importBackup(backupUri).fold(
                onSuccess = { summary ->
                    updateLastBackup(backupUri.toString(), System.currentTimeMillis())
                    android.util.Log.d("SettingsRepository", "✅ Local backup restored from: ${backupFile.absolutePath}, comics: ${summary.comicsCount}, bookmarks: ${summary.bookmarksCount}, settings: ${summary.settingsRestored}")
                },
                onFailure = { error ->
                    android.util.Log.e("SettingsRepository", "❌ Failed to restore local backup", error)
                    throw error
                }
            )
        } catch (e: Exception) {
    ...
    override suspend fun getAvailableBackups(): List<String> {
        return try {
            ...
            backupDir.listFiles { file -> file.isFile && file.name.endsWith(".mrcomic") }
                ?.map { it.name }
                ?.sortedDescending()
                ?: emptyList()
        } catch (e: Exception) {
            android.util.Log.e("SettingsRepository", "❌ Failed to get available backups", e)
            emptyList()
        }
    }

    override suspend fun createCloudBackup() {
        // This method is handled by SettingsViewModel and CloudSyncManager
    ...
    override val enableDualPageMode: Flow<Boolean> = dataStore.data.map {
        it[PreferencesKeys.ENABLE_DUAL_PAGE_MODE]?.toBoolean() ?: true
    }

    override suspend fun setEnableDualPageMode(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.ENABLE_DUAL_PAGE_MODE] = enabled.toString() }
    }

    override val enableWebtoonMode: Flow<Boolean> = dataStore.data.map {
        it[PreferencesKeys.ENABLE_WEBTOON_MODE]?.toBoolean() ?: false
    }

    override suspend fun setEnableWebtoonMode(enabled: Boolean) {
        dataStore.edit { it[PreferencesKeys.ENABLE_WEBTOON_MODE] = enabled.toString() }
    }
