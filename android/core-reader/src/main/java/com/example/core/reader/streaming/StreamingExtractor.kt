...
class StreamingExtractor(
    ...
    /**
     * Получить InputStream напрямую из архива по индексу (streaming)
     */
    suspend fun getInputStream(index: Int): Result<InputStream> = withContext(Dispatchers.IO) {
        try {
            if (index < 0 || index >= fileHeaders.size) {
                return@withContext Result.failure(IndexOutOfBoundsException("Invalid file index: $index"))
            }
            ...
            val header = fileHeaders[index]
            val archive = currentArchive ?: return@withContext Result.failure(IllegalStateException("Archive not opened"))

            // Получить InputStream напрямую из ZIP
            val inputStream = archive.getInputStream(header)
     ...
     */
    fun getFileSize(index: Int): Long? {
        return if (index in 0 until fileHeaders.size) {
            fileHeaders[index].uncompressedSize
        } else null
    }
