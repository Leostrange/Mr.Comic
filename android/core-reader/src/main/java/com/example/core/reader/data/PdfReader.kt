...
import com.example.core.reader.utils.hasPersistedReadPermission
...
class PdfReader(
    ...
    override suspend fun open(context: Context, uri: Uri): Result<MediaMetadata> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                android.util.Log.d("PdfReader", "Opening PDF file: $uri")
                
                // Проверяем и запрашиваем permissions для content:// URI
                if (uri.scheme == "content") {
                    try {
                        android.util.Log.d("PdfReader", "Checking permissions for content URI: $uri")
                        
                        val hasPermission = context.hasPersistedReadPermission(uri)
                        if (!hasPermission) {
                            android.util.Log.w("PdfReader", "No persisted permission found for URI: $uri")
                            try {
                                context.contentResolver.takePersistableUriPermission(
                                    uri,
                                    android.content.Intent.FLAG_GRANT_READ_URI_PERMISSION
                                )
                                android.util.Log.d("PdfReader", "Successfully took persistable URI permission")
                            } catch (e: SecurityException) {
                                android.util.Log.w("PdfReader", "Could not take persistable permission: ${e.message}")
                            }
                        } else {
                            android.util.Log.d("PdfReader", "Persisted permission already granted for URI")
                        }
                        
                        try {
                            context.contentResolver.openInputStream(uri)?.use {
                                android.util.Log.d("PdfReader", "Successfully opened input stream for URI")
                            } ?: run {
                                android.util.Log.e("PdfReader", "Cannot open input stream for URI")
                                return@withContext Result.failure(IllegalStateException("Нет доступа к файлу. Пожалуйста, добавьте файл заново через библиотеку."))
                            }
                        } catch (e: SecurityException) {
                            android.util.Log.e("PdfReader", "SecurityException: Cannot read URI: ${e.message}")
                            return@withContext Result.failure(IllegalStateException("Нет доступа к файлу. Пожалуйста, добавьте файл заново через библиотеку."))
                        }
                    } catch (e: Exception) {
                        android.util.Log.e("PdfReader", "Error checking permissions", e)
                    }
                }
                
                // Пытаемся открыть PDF с использованием всех доступных ридеров
                android.util.Log.d("PdfReader", "Attempting to open PDF with fallback readers")
                val result = pdfReaderFactory.openPdfWithFallback(context, uri)
                
                if (result.isFailure) {
                    val exception = result.exceptionOrNull()
                    android.util.Log.e("PdfReader", "Failed to open PDF: ${exception?.message}", exception)
                    
                    val errorMessage = when {
                        exception?.message?.contains("Permission Denial") == true -> 
                            "Нет доступа к файлу. Пожалуйста, добавьте файл заново через библиотеку."
                        exception?.message?.contains("SecurityException") == true -> 
                            "Нет доступа к файлу. Пожалуйста, добавьте файл заново через библиотеку."
                        exception?.message?.contains("FileNotFoundException") == true -> 
                            "Файл не найден. Проверьте, что файл существует."
                        else -> "Ошибка при открытии PDF файла: ${exception?.message ?: "неизвестная ошибка"}"
                    }
                    
                    return@withContext Result.failure(IllegalStateException(errorMessage, exception))
                }
                ...
                if (pageCount <= 0) {
                    android.util.Log.e("PdfReader", "PDF file contains no pages")
                    return@withContext Result.failure(IllegalStateException("PDF файл не содержит страниц"))
                }
                ...
                android.util.Log.e("PdfReader", "Exception while opening PDF", e)
                // Clean up on error
                close()
                val message = when {
    ...
    override suspend fun renderPage(
        ...
        return runCatching {
            ...
            if (primaryResult.isSuccess) {
                ...
                android.util.Log.w("PdfReader", "Primary reader returned blank page")
            }

            /*
            // Fallback с использованием PdfiumReader - DISABLED (Library missing)
            val uri = currentUri
            if (uri != null) {
                val pdfiumReader = com.example.core.reader.pdf.PdfiumReader()
                val openRes = pdfiumReader.openDocument(context, uri)
                if (openRes.isSuccess) {
                    val fbRender = pdfiumReader.renderPage(pageIndex, maxWidth, maxHeight)
                    if (fbRender.isSuccess) {
                        ...
                        renderStats.failedRenders++
                        return@runCatching Result.failure(fbRender.exceptionOrNull() ?: Exception("PdfiumReader failed to render page"))
                    }
                } else {
                    android.util.Log.w("PdfReader", "PdfiumReader open failed: ${openRes.exceptionOrNull()?.message}")
                }
            } else {
                android.util.Log.w("PdfReader", "No currentUri stored; cannot fallback to PdfiumReader")
            }
            */
    ...
    suspend fun getThumbnail(pageIndex: Int): Result<Bitmap> {
        ...
        val reader = currentReader ?: return Result.failure(IllegalStateException("PDF reader not initialized"))
