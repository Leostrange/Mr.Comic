 ...
 * Simpler, more stable, and works well with WebView's native column layout.
 *
 * Based on Readium-2/Foliate approach:
 * - Container: column-width: 100vw; column-fill: auto; height: 100vh;
 * - Page count: scrollWidth / clientWidth (horizontal scroll)
 * - Navigation: scrollLeft = pageIndex * pageWidth
 */
internal fun readerPagedCssColumnJs(targetPage: Int): String = """
(function(){
  try{
    var root=document.documentElement;
    var body=document.body;
    if(!root||!body)return JSON.stringify({handled:false,pageIndex:0,pageCount:1});

    var nativeWidth=Math.round(Number(window.__mrcomicNativeViewportWidth||0));
    var nativeHeight=Math.round(Number(window.__mrcomicNativeViewportHeight||0));
    var pageWidth=Math.max(1,nativeWidth||root.clientWidth||window.innerWidth||360);
    var pageHeight=Math.max(320,nativeHeight||window.innerHeight||640);
    var pageInsetTop=Math.max(0,parseFloat(window.__mrcomicPageInsetTop||0)||0);
    var pageInsetBottom=Math.max(0,parseFloat(window.__mrcomicPageInsetBottom||0)||0);
    var horizontalPadding=Math.max(0,parseFloat(window.__mrcomicColumnGap||0)||0);

    // Configure root for CSS columns
    root.style.setProperty('width',pageWidth+'px','important');
    root.style.setProperty('max-width',pageWidth+'px','important');
    root.style.setProperty('height',pageHeight+'px','important');
    root.style.setProperty('max-height',pageHeight+'px','important');
    ...
    // Configure body for CSS multi-column
    body.style.boxSizing='border-box';
    body.style.setProperty('width',pageWidth+'px','important');
    body.style.setProperty('max-width',pageWidth+'px','important');
    body.style.marginLeft='0';
    body.style.marginRight='0';
    body.style.setProperty('height',(pageHeight-pageInsetTop-pageInsetBottom)+'px','important');
    body.style.setProperty('overflow','hidden','important');
    body.style.setProperty('padding-top',pageInsetTop+'px','important');
    body.style.setProperty('padding-bottom',pageInsetBottom+'px','important');
    body.style.setProperty('padding-left',Math.floor(horizontalPadding/2)+'px','important');
    body.style.setProperty('padding-right',Math.ceil(horizontalPadding/2)+'px','important');

    // CSS multi-column layout
    body.style.setProperty('-webkit-column-width',pageWidth+'px','important');
    ...
    // Calculate pages from scrollWidth
    var scrollWidth=Math.max(body.scrollWidth||0,body.offsetWidth||0);
    var pageCount=Math.max(1,Math.ceil(scrollWidth/pageWidth));
    window.__mrcomicPageCount=pageCount;
    window.__mrcomicPageWidth=pageWidth;

    // Navigate to target page
    var target=Math.max(0,Math.min(pageCount-1,$targetPage||0));
    body.scrollLeft=target*pageWidth;
    window.__mrcomicPagedIndex=target;

    // Remove paged viewport/content wrappers if they exist
    var pagedViewport=document.getElementById('__mrcomic_paged_viewport');
    var pagedContent=document.getElementById('__mrcomic_paged_content');
    if(pagedViewport&&pagedContent&&pagedContent.parentNode===pagedViewport){
      Array.prototype.slice.call(pagedContent.childNodes).forEach(function(node){
        document.body.insertBefore(node,pagedViewport);
      });
      if(pagedViewport.parentNode)pagedViewport.parentNode.removeChild(pagedViewport);
    }
...
/**
 * CSS Multi-Column page turn — navigates between pages using scrollLeft.
 */
internal fun readerPagedCssColumnTurnJs(delta: Int): String = """
(function(){
  try{
    var body=document.body;
    var pageWidth=window.__mrcomicPageWidth||body.clientWidth||360;
    var pageCount=window.__mrcomicPageCount||1;
    var current=Math.round(body.scrollLeft/pageWidth)||0;
    var target=current+($delta);
    if(target<0||target>=pageCount){
      return JSON.stringify({handled:false,pageIndex:current,pageCount:pageCount});
    }
