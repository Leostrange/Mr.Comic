...
class ReaderSettingsViewModel @Inject constructor(
    ...
    fun onEnableDualPageModeChanged(enabled: Boolean) {
        viewModelScope.launch { settingsRepository.setEnableDualPageMode(enabled) }
    }

    fun onEnableWebtoonModeChanged(enabled: Boolean) {
        viewModelScope.launch { settingsRepository.setEnableWebtoonMode(enabled) }
    }
}
