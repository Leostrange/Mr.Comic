...
internal const val READER_USER_FONT_ASSET_PATH = "/reader/fonts/"
private const val READER_USER_FONT_BASE_URL = "https://appassets.androidplatform.net/reader/fonts/"

internal data class ReaderResolvedTextFont(
...
object ReaderTextFontCatalog {
    ...
    internal fun resolve(context: Context, selectedFamily: String?): ReaderResolvedTextFont {
        ...
        if (builtInFonts.containsKey(requestedFamily)) {
            return ReaderResolvedTextFont(
                ...
                sourceUrl = builtInAsset?.let { "file:///android_asset/fonts/$it" }
            )
        }
    ...
    internal fun openCustomFontAsset(context: Context, path: String): FormatReaderWebResource? {
        val decodedPath = Uri.decode(path).replace('\\', '/').trimStart('/')
        val fileName = decodedPath.substringAfterLast('/').trim()
        if (fileName.isBlank()) return null
        val rootDir = fontDirectory(context).canonicalFile
        val file = File(rootDir, fileName).canonicalFile
        if (file.parentFile != rootDir || !file.exists() || !file.isFile || !file.canRead()) {
            return null
        }
        val mimeType = supportedFontMimeTypes[file.extension.lowercase(Locale.US)] ?: return null
    ...
    internal fun prettifyFontDisplayName(fileName: String): String {
