...
class ReaderBottomSheetHostTest {
    ...
    @Test
    fun dataClass_equalsAndHashCode_compareAllFields() {
        val a: Any? = emptyHost()
        val b: Any? = emptyHost()

        // Relaxed mocks intentionally represent different dependency instances;
        // assert the stable structural contract rather than mock identity equality.
        assertTrue(a.toString().contains("ReaderBottomSheetHost"))
        assertTrue(a.toString().contains("showReaderAudioSheet=false"))
        assertTrue(b.toString().contains("showReaderAudioSheet=false"))
    }
