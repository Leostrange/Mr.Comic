package com.example.mrcomic.data

/**
 * Модель данных комикса
 */
data class Comic(
    val id: Int,
    val title: String,
    val author: String = "",
    val pageCount: Int,
    val currentPage: Int = 0,
    val coverPath: String? = null,
    val filePath: String? = null,
    val isFavorite: Boolean = false,
    val description: String? = null
) {
    fun getImagePath(page: Int): String {
        // Здесь должна быть реальная логика получения пути к изображению
        return "android.resource://com.example.mrcomic/drawable/comic_${id}_page_$page"
    }
    
    fun getProgress(): Float {
        return if (pageCount > 0) currentPage.toFloat() / pageCount.toFloat() else 0f
    }
    
    fun hasNextPage(): Boolean = currentPage < pageCount - 1
    fun hasPreviousPage(): Boolean = currentPage > 0
}