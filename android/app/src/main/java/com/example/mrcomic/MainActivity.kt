    ...
    MrComicTheme(themeConfig = themeConfig) {
        Surface(
            ...
            if (showVideoSplash) {
                ...
                // Автоматически выбирает горизонтальное видео для landscape режима
                ...
                when {
                    ...
                    showError != null -> {
                        ...
                        // Определяем стартовый экран на основе статуса онбординга
                        val userPreferences = remember { UserPreferences(context.dataStore) }
                        val isOnboardingCompleted by userPreferences.get(PreferencesKeys.APP_ONBOARDING_COMPLETED, false).collectAsState(initial = false)
                        val startDestination = if (isOnboardingCompleted) Screen.Library.route else Screen.Onboarding.route
