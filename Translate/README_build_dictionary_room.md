...
## Room-compatible build

If you want to open the generated DB directly with Android Room, use `build_dictionary_room.py`.
That variant stores `entries.pos` as non-null text, uses a normal composite unique index on
`(lang, normalized_lemma, pos, source)`, and sets `PRAGMA user_version = 1`.
