...
ROOM_IDENTITY_HASH = "8bff8dc2a932e4d7634a06a769328bef"
DEFAULT_URLS = {
...
def parse_kaikki_args(values: list[str]) -> dict[str, str]:
    mapping: dict[str, str] = {}
    for item in values:
        if ":" not in item:
            raise ValueError(f"--kaikki value must have lang:path-or-url form, got: {item}")
        raw_lang, raw_path = item.split(":", 1)
        lang = normalize_lang_code(raw_lang)
        if lang is None:
            raise ValueError(f"Unsupported or invalid language code in --kaikki: {raw_lang}")
        path_or_url = raw_path.strip()
        if not path_or_url:
            raise ValueError(f"Missing path/url for --kaikki {raw_lang}")
        mapping[lang] = path_or_url
    return mapping
...
class DictionaryDB:
    ...
    def _reset_existing_db_files(self) -> None:
        for candidate in [
            self.path,
    ...
    def _create_schema(self) -> None:
        ...
        cur.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_forms_entry_id
              ON forms(entry_id);
            ...
            CREATE INDEX IF NOT EXISTS idx_senses_entry_id
              ON senses(entry_id);
            ...
            CREATE INDEX IF NOT EXISTS idx_readings_entry_id
            ...
            CREATE INDEX IF NOT EXISTS idx_examples_entry_id
    ...
    def insert_entry(self, payload: EntryPayload) -> None:
        ...
        cur.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (payload.lang, lemma, normalized_lemma, payload.pos or '', payload.source),
        )
        cur.execute(
            """
            ...
            WHERE lang = ? AND normalized_lemma = ?
              ...
              AND source = ?
            """,
            (payload.lang, normalized_lemma, payload.pos or '', payload.source),
        )
        row = cur.fetchone()
        if row is None:
...
def build_from_args(args: argparse.Namespace) -> dict[str, dict[str, int]]:
    ...
    generic_kaikki_sources = {
        lang: resolve_source(
            path_or_url,
            path_or_url,
            sources_dir / f"{lang}wiktionary.jsonl.gz",
            args.download_missing
        )
        for lang, path_or_url in parse_kaikki_args(args.kaikki).items()
    }
    ...
    try:
        ...
        for lang, path in generic_kaikki_sources.items():
            if path is None:
                continue
            import_stream(
                router,
                parse_kaikki_jsonl(path, {lang}, "kaikki", limit=args.limit),
                f"Kaikki {lang.upper()}"
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--kaikki", action="append", default=[], help="Additional Kaikki source in lang:path-or-url form (repeatable)")
