...
Generated for nearly every Kotlin coroutine `SuspendLambda`:
...
bash .agents/skills/android-reverse-engineering/scripts/recover-kotlin-names.sh <output>/sources [mapping-dir]
...
# Query the mapping:
bash .agents/skills/android-reverse-engineering/scripts/lookup-name.sh <mapping-dir> Repository                 # search
bash .agents/skills/android-reverse-engineering/scripts/lookup-name.sh <mapping-dir> -o ab.cd                   # obf -> real
bash .agents/skills/android-reverse-engineering/scripts/lookup-name.sh <mapping-dir> -p com.example.feature     # list package
bash .agents/skills/android-reverse-engineering/scripts/lookup-name.sh <mapping-dir> --grep '"api/' <output>/sources
...
map back to the names the developer actually wrote.

## Limitations

- **Method names and field names** are not recovered. Kotlin metadata only
  preserves class-level FQNs.
- **Pure-Java classes** carry no `@Metadata`, so they remain obfuscated.
- **Heavily inlined classes** sometimes show up under the wrong filename.
...
3. When you hit an obfuscated FQN in code (e.g. `nq.e`), resolve it with
   `lookup-name.sh <mapping-dir> -o nq.e`.
