...
1. **Use `--deobf`** to generate readable replacement names:
   ...
   ```
   jadx creates a mapping file at `output-dir/deobf-mapping.txt`.

2. **Use the ProGuard mapping file** if available:
   ```bash
   jadx --deobf-map mapping.txt -d output-dir app.apk
   ```

3. **Focus on string constants and API calls** rather than class names. URL strings, annotation values, and library classes are not obfuscated.
