...
The `Application` subclass initializes global singletons:
...
- **Android framework classes** — `Activity`, `Fragment` keep their names
...
3. **Follow library calls**: Retrofit `@GET`/`@POST` annotations are readable
4. **Recover original Kotlin names from metadata**: Run `recover-kotlin-names.sh`
