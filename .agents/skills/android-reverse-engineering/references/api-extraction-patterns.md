...
Ktor is the dominant HTTP client in Kotlin Multiplatform and modern Kotlin-only Android apps. Unlike Retrofit, Ktor does **not** use annotations to declare endpoints — paths appear as plain string arguments.
...
**Note on obfuscation:** In heavily R8-shrunk apps the call site is inlined and the `client.<verb>(` regex misses it. The path string itself is **not** obfuscated — fall back to the generic path-literal search (`--paths`) for the endpoint inventory.
...
When the HTTP client cannot be identified or call sites are obfuscated:
