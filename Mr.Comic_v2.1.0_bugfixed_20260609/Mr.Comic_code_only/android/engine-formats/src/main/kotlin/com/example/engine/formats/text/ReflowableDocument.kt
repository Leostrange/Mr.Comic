...
internal object ReflowableDocumentBuilder {
    ...
    private fun mergeTrailingOrphanHtmlChunks(chunks: List<String>): List<String> {
        ...
        if (last.visibleTextLength() in 1 until MIN_ORPHAN_TEXT_CHARS) {
            val previousIndex = merged.lastIndex - 1
            // Extract the inner text from the orphan chunk and append it to the
            // previous chunk's closing tag, instead of raw concatenation which
            // duplicates opening/closing tags and produces invalid HTML.
            val previous = merged[previousIndex]
            val orphanText = runCatching {
                Jsoup.parseBodyFragment(last).body().text().trim()
            }.getOrNull().orEmpty()
            if (orphanText.isNotBlank()) {
                val insertionPoint = previous.lastIndexOf("</")
                if (insertionPoint > 0) {
                    val escapedOrphan = escapeHtml(orphanText)
                    merged[previousIndex] = previous.substring(0, insertionPoint) +
                        " $escapedOrphan" + previous.substring(insertionPoint)
                } else {
                    merged[previousIndex] = previous + " " + escapeHtml(orphanText)
                }
            }
            merged.removeAt(merged.lastIndex)
        }
