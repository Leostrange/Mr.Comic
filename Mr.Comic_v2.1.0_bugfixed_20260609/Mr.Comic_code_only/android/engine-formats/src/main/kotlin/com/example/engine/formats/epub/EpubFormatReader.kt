...
class EpubFormatReader(
    ...
    companion object {
        ...
        private const val EPUB_STRUCTURE_CACHE_VERSION = 8
        ...
        while (i < rawResult.size) {
            ...
            if (pg is EpubPage.Html && pg.totalChunks == 1 && isNotesTitlePage(zip, pg.entry)) {
                ...
                if (noteEntries.isNotEmpty()) {
                    // Build synthetic note pages for footnoteMap population, but do NOT
                    // add them to the page list. Footnotes are accessed via popup when
                    // the user taps a footnote reference — they should not appear as
                    // regular pages that interrupt reading flow.
                    buildSyntheticNotePages(pg.entry, noteEntries, zip)
                    i = j
                    continue
                }
            }
    ...
    private fun buildSyntheticNotePages(titleEntry: String, noteEntries: List<String>, zip: ZipFile): List<EpubPage> {
        ...
        for ((sourceEntry, item) in noteItems) {
            ...
            // Render as popup-style footnote: collapsible note item with data attribute
            // so the reader JS can display it as an overlay/popup instead of a full page.
            val html = """<div class="note-item" id="$escapedId" data-mrcomic-footnote="true"><span class="note-num">$escapedNum</span><span class="note-text">$escapedText</span></div>"""
