...
import com.example.engine.formats.epub.detectEpubTextCharset
...
internal fun decodeTextBytes(bytes: ByteArray): String {
    ...
    // Use the EPUB-level charset detection which includes declared-encoding probing,
    // BOM detection, strict UTF-8 validation, and a broader fallback charset set
    // (including CJK and Korean encodings) with a more sophisticated scoring function.
    // This unifies charset detection across all text-based formats.
    val charset = detectEpubTextCharset(bytes)
    val decoded = bytes.toString(charset).removePrefix("\uFEFF")
