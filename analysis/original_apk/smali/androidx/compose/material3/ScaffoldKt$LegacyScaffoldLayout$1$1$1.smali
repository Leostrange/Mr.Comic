.class final Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;
.super Lkotlin/jvm/internal/Lambda;
.source "Scaffold.kt"

# interfaces
.implements Lkotlin/jvm/functions/Function1;


# annotations
.annotation system Ldalvik/annotation/EnclosingMethod;
    value = Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1;->invoke-0kLqBqw(Landroidx/compose/ui/layout/SubcomposeMeasureScope;J)Landroidx/compose/ui/layout/MeasureResult;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x18
    name = null
.end annotation

.annotation system Ldalvik/annotation/Signature;
    value = {
        "Lkotlin/jvm/internal/Lambda;",
        "Lkotlin/jvm/functions/Function1<",
        "Landroidx/compose/ui/layout/Placeable$PlacementScope;",
        "Lkotlin/Unit;",
        ">;"
    }
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nScaffold.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Scaffold.kt\nandroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1\n+ 2 ListUtils.kt\nandroidx/compose/ui/util/ListUtilsKt\n+ 3 fake.kt\nkotlin/jvm/internal/FakeKt\n*L\n1#1,598:1\n151#2,3:599\n33#2,4:602\n154#2,2:606\n38#2:608\n156#2:609\n171#2,13:610\n151#2,3:623\n33#2,4:626\n154#2,2:630\n38#2:632\n156#2:633\n171#2,13:634\n171#2,13:647\n416#2,3:660\n33#2,4:663\n419#2:667\n420#2:670\n38#2:671\n421#2:672\n171#2,13:673\n171#2,13:686\n151#2,3:699\n33#2,4:702\n154#2,2:706\n38#2:708\n156#2:709\n171#2,13:710\n151#2,3:723\n33#2,4:726\n154#2,2:730\n38#2:732\n156#2:733\n33#2,6:734\n33#2,6:740\n33#2,6:746\n33#2,6:752\n33#2,6:758\n1#3:668\n1#3:669\n*S KotlinDebug\n*F\n+ 1 Scaffold.kt\nandroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1\n*L\n356#1:599,3\n356#1:602,4\n356#1:606,2\n356#1:608\n356#1:609\n360#1:610,13\n362#1:623,3\n362#1:626,4\n362#1:630,2\n362#1:632\n362#1:633\n378#1:634,13\n379#1:647,13\n382#1:660,3\n382#1:663,4\n382#1:667\n382#1:670\n382#1:671\n382#1:672\n399#1:673,13\n400#1:686,13\n434#1:699,3\n434#1:702,4\n434#1:706,2\n434#1:708\n434#1:709\n436#1:710,13\n475#1:723,3\n475#1:726,4\n475#1:730,2\n475#1:732\n475#1:733\n478#1:734,6\n481#1:740,6\n484#1:746,6\n492#1:752,6\n497#1:758,6\n382#1:669\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000\u000c\n\u0000\n\u0002\u0010\u0002\n\u0002\u0018\u0002\n\u0000\u0010\u0000\u001a\u00020\u0001*\u00020\u0002H\n\u00a2\u0006\u0002\u0008\u0003"
    }
    d2 = {
        "<anonymous>",
        "",
        "Landroidx/compose/ui/layout/Placeable$PlacementScope;",
        "invoke"
    }
    k = 0x3
    mv = {
        0x1,
        0x8,
        0x0
    }
    xi = 0x30
.end annotation


# instance fields
.field final synthetic $bottomBar:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $content:Lkotlin/jvm/functions/Function3;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function3<",
            "Landroidx/compose/foundation/layout/PaddingValues;",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

.field final synthetic $fab:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $fabPosition:I

.field final synthetic $layoutHeight:I

.field final synthetic $layoutWidth:I

.field final synthetic $looseConstraints:J

.field final synthetic $snackbar:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field final synthetic $this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

.field final synthetic $topBar:Lkotlin/jvm/functions/Function2;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function2<",
            "Landroidx/compose/runtime/Composer;",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method constructor <init>(Landroidx/compose/ui/layout/SubcomposeMeasureScope;Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function2;IILandroidx/compose/foundation/layout/WindowInsets;JLkotlin/jvm/functions/Function2;Lkotlin/jvm/functions/Function3;I)V
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/ui/layout/SubcomposeMeasureScope;",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;II",
            "Landroidx/compose/foundation/layout/WindowInsets;",
            "J",
            "Lkotlin/jvm/functions/Function2<",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function3<",
            "-",
            "Landroidx/compose/foundation/layout/PaddingValues;",
            "-",
            "Landroidx/compose/runtime/Composer;",
            "-",
            "Ljava/lang/Integer;",
            "Lkotlin/Unit;",
            ">;I)V"
        }
    .end annotation

    iput-object p1, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    iput-object p2, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$topBar:Lkotlin/jvm/functions/Function2;

    iput-object p3, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$snackbar:Lkotlin/jvm/functions/Function2;

    iput-object p4, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$fab:Lkotlin/jvm/functions/Function2;

    iput p5, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$fabPosition:I

    iput p6, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutWidth:I

    iput-object p7, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    iput-wide p8, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$looseConstraints:J

    iput-object p10, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$bottomBar:Lkotlin/jvm/functions/Function2;

    iput-object p11, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$content:Lkotlin/jvm/functions/Function3;

    iput p12, p0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutHeight:I

    const/4 v0, 0x1

    invoke-direct {p0, v0}, Lkotlin/jvm/internal/Lambda;-><init>(I)V

    return-void
.end method


# virtual methods
.method public bridge synthetic invoke(Ljava/lang/Object;)Ljava/lang/Object;
    .locals 1
    .param p1, "p1"    # Ljava/lang/Object;

    .line 355
    move-object v0, p1

    check-cast v0, Landroidx/compose/ui/layout/Placeable$PlacementScope;

    invoke-virtual {p0, v0}, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->invoke(Landroidx/compose/ui/layout/Placeable$PlacementScope;)V

    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    return-object v0
.end method

.method public final invoke(Landroidx/compose/ui/layout/Placeable$PlacementScope;)V
    .locals 34
    .param p1, "$this$layout"    # Landroidx/compose/ui/layout/Placeable$PlacementScope;

    .line 356
    move-object/from16 v0, p0

    iget-object v1, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    sget-object v2, Landroidx/compose/material3/ScaffoldLayoutContent;->TopBar:Landroidx/compose/material3/ScaffoldLayoutContent;

    iget-object v3, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$topBar:Lkotlin/jvm/functions/Function2;

    invoke-interface {v1, v2, v3}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v1

    .local v1, "$this$fastMap$iv":Ljava/util/List;
    iget-wide v2, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$looseConstraints:J

    const/4 v4, 0x0

    .line 599
    .local v4, "$i$f$fastMap":I
    nop

    .line 600
    new-instance v5, Ljava/util/ArrayList;

    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v6

    invoke-direct {v5, v6}, Ljava/util/ArrayList;-><init>(I)V

    .line 601
    .local v5, "target$iv":Ljava/util/ArrayList;
    move-object v6, v1

    .local v6, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 602
    .local v7, "$i$f$fastForEach":I
    nop

    .line 603
    const/4 v8, 0x0

    .local v8, "index$iv$iv":I
    invoke-interface {v6}, Ljava/util/List;->size()I

    move-result v9

    :goto_0
    if-ge v8, v9, :cond_0

    .line 604
    invoke-interface {v6, v8}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v10

    .line 605
    .local v10, "item$iv$iv":Ljava/lang/Object;
    move-object v11, v10

    .local v11, "it$iv":Ljava/lang/Object;
    const/4 v12, 0x0

    .line 606
    .local v12, "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move-object v13, v5

    check-cast v13, Ljava/util/Collection;

    move-object v14, v11

    check-cast v14, Landroidx/compose/ui/layout/Measurable;

    .local v14, "it":Landroidx/compose/ui/layout/Measurable;
    const/4 v15, 0x0

    .line 357
    .local v15, "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$topBarPlaceables$1":I
    invoke-interface {v14, v2, v3}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v14

    .line 606
    .end local v14    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v15    # "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$topBarPlaceables$1":I
    invoke-interface {v13, v14}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 607
    nop

    .line 605
    .end local v11    # "it$iv":Ljava/lang/Object;
    .end local v12    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 603
    .end local v10    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v8, v8, 0x1

    goto :goto_0

    .line 608
    .end local v8    # "index$iv$iv":I
    :cond_0
    nop

    .line 609
    .end local v6    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v7    # "$i$f$fastForEach":I
    move-object v13, v5

    check-cast v13, Ljava/util/List;

    .line 356
    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .end local v4    # "$i$f$fastMap":I
    .end local v5    # "target$iv":Ljava/util/ArrayList;
    nop

    .line 360
    .local v13, "topBarPlaceables":Ljava/util/List;
    move-object v1, v13

    .local v1, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v2, 0x0

    .line 610
    .local v2, "$i$f$fastMaxBy":I
    nop

    .line 611
    invoke-interface {v1}, Ljava/util/List;->isEmpty()Z

    move-result v3

    const/4 v5, 0x0

    if-eqz v3, :cond_1

    const/4 v3, 0x0

    goto :goto_2

    .line 612
    :cond_1
    invoke-interface {v1, v5}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v3

    .line 613
    .local v3, "maxElem$iv":Ljava/lang/Object;
    move-object v6, v3

    check-cast v6, Landroidx/compose/ui/layout/Placeable;

    .local v6, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v7, 0x0

    .line 360
    .local v7, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$topBarHeight$1":I
    invoke-virtual {v6}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v6

    .line 613
    .end local v6    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v7    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$topBarHeight$1":I
    nop

    .line 614
    .local v6, "maxValue$iv":I
    const/4 v7, 0x1

    .local v7, "i$iv":I
    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v8

    if-gt v7, v8, :cond_3

    .line 615
    :goto_1
    invoke-interface {v1, v7}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v9

    .line 616
    .local v9, "e$iv":Ljava/lang/Object;
    move-object v10, v9

    check-cast v10, Landroidx/compose/ui/layout/Placeable;

    .local v10, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v11, 0x0

    .line 360
    .local v11, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$topBarHeight$1":I
    invoke-virtual {v10}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v10

    .line 616
    .end local v10    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v11    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$topBarHeight$1":I
    nop

    .line 617
    .local v10, "v$iv":I
    if-ge v6, v10, :cond_2

    .line 618
    move-object v3, v9

    .line 619
    move v6, v10

    .line 614
    .end local v9    # "e$iv":Ljava/lang/Object;
    .end local v10    # "v$iv":I
    :cond_2
    if-eq v7, v8, :cond_3

    add-int/lit8 v7, v7, 0x1

    goto :goto_1

    .line 622
    .end local v7    # "i$iv":I
    :cond_3
    nop

    .line 360
    .end local v1    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v2    # "$i$f$fastMaxBy":I
    .end local v3    # "maxElem$iv":Ljava/lang/Object;
    .end local v6    # "maxValue$iv":I
    :goto_2
    check-cast v3, Landroidx/compose/ui/layout/Placeable;

    if-eqz v3, :cond_4

    invoke-virtual {v3}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v1

    move v14, v1

    goto :goto_3

    :cond_4
    move v14, v5

    .line 362
    .local v14, "topBarHeight":I
    :goto_3
    iget-object v1, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    sget-object v2, Landroidx/compose/material3/ScaffoldLayoutContent;->Snackbar:Landroidx/compose/material3/ScaffoldLayoutContent;

    iget-object v3, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$snackbar:Lkotlin/jvm/functions/Function2;

    invoke-interface {v1, v2, v3}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v1

    .local v1, "$this$fastMap$iv":Ljava/util/List;
    iget-object v2, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    iget-object v3, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    iget-wide v6, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$looseConstraints:J

    const/4 v8, 0x0

    .line 623
    .local v8, "$i$f$fastMap":I
    nop

    .line 624
    new-instance v9, Ljava/util/ArrayList;

    invoke-interface {v1}, Ljava/util/List;->size()I

    move-result v10

    invoke-direct {v9, v10}, Ljava/util/ArrayList;-><init>(I)V

    .line 625
    .local v9, "target$iv":Ljava/util/ArrayList;
    move-object v10, v1

    .local v10, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v11, 0x0

    .line 626
    .local v11, "$i$f$fastForEach":I
    nop

    .line 627
    const/4 v12, 0x0

    .local v12, "index$iv$iv":I
    invoke-interface {v10}, Ljava/util/List;->size()I

    move-result v15

    :goto_4
    if-ge v12, v15, :cond_5

    .line 628
    invoke-interface {v10, v12}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v16

    .line 629
    .local v16, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v17, v16

    .local v17, "it$iv":Ljava/lang/Object;
    const/16 v18, 0x0

    .line 630
    .local v18, "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move-object v4, v9

    check-cast v4, Ljava/util/Collection;

    move-object/from16 v5, v17

    check-cast v5, Landroidx/compose/ui/layout/Measurable;

    .local v5, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v21, 0x0

    .line 364
    .local v21, "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarPlaceables$1":I
    nop

    .line 365
    move-object/from16 v22, v1

    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .local v22, "$this$fastMap$iv":Ljava/util/List;
    move-object v1, v3

    check-cast v1, Landroidx/compose/ui/unit/Density;

    move/from16 v23, v8

    .end local v8    # "$i$f$fastMap":I
    .local v23, "$i$f$fastMap":I
    invoke-interface {v3}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v8

    invoke-interface {v2, v1, v8}, Landroidx/compose/foundation/layout/WindowInsets;->getLeft(Landroidx/compose/ui/unit/Density;Landroidx/compose/ui/unit/LayoutDirection;)I

    move-result v8

    .line 364
    nop

    .line 366
    .local v8, "leftInset":I
    nop

    .line 367
    move-object/from16 v24, v10

    .end local v10    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .local v24, "$this$fastForEach$iv$iv":Ljava/util/List;
    invoke-interface {v3}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v10

    invoke-interface {v2, v1, v10}, Landroidx/compose/foundation/layout/WindowInsets;->getRight(Landroidx/compose/ui/unit/Density;Landroidx/compose/ui/unit/LayoutDirection;)I

    move-result v10

    .line 366
    nop

    .line 368
    .local v10, "rightInset":I
    invoke-interface {v2, v1}, Landroidx/compose/foundation/layout/WindowInsets;->getBottom(Landroidx/compose/ui/unit/Density;)I

    move-result v1

    .line 370
    .local v1, "bottomInset":I
    nop

    .line 371
    nop

    .line 372
    move-object/from16 v25, v2

    neg-int v2, v8

    sub-int/2addr v2, v10

    .line 373
    move-object/from16 v26, v3

    neg-int v3, v1

    .line 371
    invoke-static {v6, v7, v2, v3}, Landroidx/compose/ui/unit/ConstraintsKt;->offset-NN6Ew-U(JII)J

    move-result-wide v2

    .line 370
    invoke-interface {v5, v2, v3}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v1

    .line 630
    .end local v1    # "bottomInset":I
    .end local v5    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v8    # "leftInset":I
    .end local v10    # "rightInset":I
    .end local v21    # "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarPlaceables$1":I
    invoke-interface {v4, v1}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 631
    nop

    .line 629
    .end local v17    # "it$iv":Ljava/lang/Object;
    .end local v18    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 627
    .end local v16    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v12, v12, 0x1

    move-object/from16 v1, v22

    move/from16 v8, v23

    move-object/from16 v10, v24

    move-object/from16 v2, v25

    move-object/from16 v3, v26

    const/4 v5, 0x0

    goto :goto_4

    .end local v22    # "$this$fastMap$iv":Ljava/util/List;
    .end local v23    # "$i$f$fastMap":I
    .end local v24    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .local v1, "$this$fastMap$iv":Ljava/util/List;
    .local v8, "$i$f$fastMap":I
    .local v10, "$this$fastForEach$iv$iv":Ljava/util/List;
    :cond_5
    move-object/from16 v22, v1

    move/from16 v23, v8

    move-object/from16 v24, v10

    .line 632
    .end local v1    # "$this$fastMap$iv":Ljava/util/List;
    .end local v8    # "$i$f$fastMap":I
    .end local v10    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v12    # "index$iv$iv":I
    .restart local v22    # "$this$fastMap$iv":Ljava/util/List;
    .restart local v23    # "$i$f$fastMap":I
    .restart local v24    # "$this$fastForEach$iv$iv":Ljava/util/List;
    nop

    .line 633
    .end local v11    # "$i$f$fastForEach":I
    .end local v24    # "$this$fastForEach$iv$iv":Ljava/util/List;
    move-object v1, v9

    check-cast v1, Ljava/util/List;

    .line 362
    .end local v9    # "target$iv":Ljava/util/ArrayList;
    .end local v22    # "$this$fastMap$iv":Ljava/util/List;
    .end local v23    # "$i$f$fastMap":I
    nop

    .line 378
    .local v1, "snackbarPlaceables":Ljava/util/List;
    move-object v2, v1

    .local v2, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v3, 0x0

    .line 634
    .local v3, "$i$f$fastMaxBy":I
    nop

    .line 635
    invoke-interface {v2}, Ljava/util/List;->isEmpty()Z

    move-result v4

    if-eqz v4, :cond_6

    const/4 v5, 0x0

    goto :goto_6

    .line 636
    :cond_6
    const/4 v4, 0x0

    invoke-interface {v2, v4}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v5

    .line 637
    .local v5, "maxElem$iv":Ljava/lang/Object;
    move-object v4, v5

    check-cast v4, Landroidx/compose/ui/layout/Placeable;

    .local v4, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v6, 0x0

    .line 378
    .local v6, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarHeight$1":I
    invoke-virtual {v4}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v4

    .line 637
    .end local v4    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v6    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarHeight$1":I
    nop

    .line 638
    .local v4, "maxValue$iv":I
    const/4 v6, 0x1

    .local v6, "i$iv":I
    invoke-static {v2}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v7

    if-gt v6, v7, :cond_8

    .line 639
    :goto_5
    invoke-interface {v2, v6}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v8

    .line 640
    .local v8, "e$iv":Ljava/lang/Object;
    move-object v9, v8

    check-cast v9, Landroidx/compose/ui/layout/Placeable;

    .local v9, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v10, 0x0

    .line 378
    .local v10, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarHeight$1":I
    invoke-virtual {v9}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v9

    .line 640
    .end local v9    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v10    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarHeight$1":I
    nop

    .line 641
    .local v9, "v$iv":I
    if-ge v4, v9, :cond_7

    .line 642
    move-object v5, v8

    .line 643
    move v4, v9

    .line 638
    .end local v8    # "e$iv":Ljava/lang/Object;
    .end local v9    # "v$iv":I
    :cond_7
    if-eq v6, v7, :cond_8

    add-int/lit8 v6, v6, 0x1

    goto :goto_5

    .line 646
    .end local v6    # "i$iv":I
    :cond_8
    nop

    .line 378
    .end local v2    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v3    # "$i$f$fastMaxBy":I
    .end local v4    # "maxValue$iv":I
    .end local v5    # "maxElem$iv":Ljava/lang/Object;
    :goto_6
    check-cast v5, Landroidx/compose/ui/layout/Placeable;

    if-eqz v5, :cond_9

    invoke-virtual {v5}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v2

    goto :goto_7

    :cond_9
    const/4 v2, 0x0

    .line 379
    .local v2, "snackbarHeight":I
    :goto_7
    move-object v3, v1

    .local v3, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v4, 0x0

    .line 647
    .local v4, "$i$f$fastMaxBy":I
    nop

    .line 648
    invoke-interface {v3}, Ljava/util/List;->isEmpty()Z

    move-result v5

    if-eqz v5, :cond_a

    const/4 v6, 0x0

    goto :goto_9

    .line 649
    :cond_a
    const/4 v5, 0x0

    invoke-interface {v3, v5}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v6

    .line 650
    .local v6, "maxElem$iv":Ljava/lang/Object;
    move-object v5, v6

    check-cast v5, Landroidx/compose/ui/layout/Placeable;

    .local v5, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v7, 0x0

    .line 379
    .local v7, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarWidth$1":I
    invoke-virtual {v5}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v5

    .line 650
    .end local v5    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v7    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarWidth$1":I
    nop

    .line 651
    .local v5, "maxValue$iv":I
    const/4 v7, 0x1

    .local v7, "i$iv":I
    invoke-static {v3}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v8

    if-gt v7, v8, :cond_c

    .line 652
    :goto_8
    invoke-interface {v3, v7}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v9

    .line 653
    .local v9, "e$iv":Ljava/lang/Object;
    move-object v10, v9

    check-cast v10, Landroidx/compose/ui/layout/Placeable;

    .local v10, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v11, 0x0

    .line 379
    .local v11, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarWidth$1":I
    invoke-virtual {v10}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v10

    .line 653
    .end local v10    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v11    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$snackbarWidth$1":I
    nop

    .line 654
    .local v10, "v$iv":I
    if-ge v5, v10, :cond_b

    .line 655
    move-object v6, v9

    .line 656
    move v5, v10

    .line 651
    .end local v9    # "e$iv":Ljava/lang/Object;
    .end local v10    # "v$iv":I
    :cond_b
    if-eq v7, v8, :cond_c

    add-int/lit8 v7, v7, 0x1

    goto :goto_8

    .line 659
    .end local v7    # "i$iv":I
    :cond_c
    nop

    .line 379
    .end local v3    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v4    # "$i$f$fastMaxBy":I
    .end local v5    # "maxValue$iv":I
    .end local v6    # "maxElem$iv":Ljava/lang/Object;
    :goto_9
    check-cast v6, Landroidx/compose/ui/layout/Placeable;

    if-eqz v6, :cond_d

    invoke-virtual {v6}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v3

    goto :goto_a

    :cond_d
    const/4 v3, 0x0

    .line 382
    .local v3, "snackbarWidth":I
    :goto_a
    iget-object v4, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    sget-object v5, Landroidx/compose/material3/ScaffoldLayoutContent;->Fab:Landroidx/compose/material3/ScaffoldLayoutContent;

    iget-object v6, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$fab:Lkotlin/jvm/functions/Function2;

    invoke-interface {v4, v5, v6}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v4

    .local v4, "$this$fastMapNotNull$iv":Ljava/util/List;
    iget-object v5, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    iget-object v6, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    iget-wide v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$looseConstraints:J

    const/4 v9, 0x0

    .line 660
    .local v9, "$i$f$fastMapNotNull":I
    nop

    .line 661
    new-instance v10, Ljava/util/ArrayList;

    invoke-interface {v4}, Ljava/util/List;->size()I

    move-result v11

    invoke-direct {v10, v11}, Ljava/util/ArrayList;-><init>(I)V

    .line 662
    .local v10, "target$iv":Ljava/util/ArrayList;
    move-object v11, v4

    .local v11, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v12, 0x0

    .line 663
    .local v12, "$i$f$fastForEach":I
    nop

    .line 664
    const/4 v15, 0x0

    move-object/from16 v16, v4

    .end local v4    # "$this$fastMapNotNull$iv":Ljava/util/List;
    .local v15, "index$iv$iv":I
    .local v16, "$this$fastMapNotNull$iv":Ljava/util/List;
    invoke-interface {v11}, Ljava/util/List;->size()I

    move-result v4

    :goto_b
    move/from16 v17, v9

    .end local v9    # "$i$f$fastMapNotNull":I
    .local v17, "$i$f$fastMapNotNull":I
    if-ge v15, v4, :cond_11

    .line 665
    invoke-interface {v11, v15}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v18

    .line 666
    .local v18, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v21, v18

    .local v21, "e$iv":Ljava/lang/Object;
    const/16 v22, 0x0

    .line 667
    .local v22, "$i$a$-fastForEach-ListUtilsKt$fastMapNotNull$2$iv":I
    move-object/from16 v9, v21

    check-cast v9, Landroidx/compose/ui/layout/Measurable;

    .local v9, "measurable":Landroidx/compose/ui/layout/Measurable;
    const/16 v24, 0x0

    .line 385
    .local v24, "$i$a$-fastMapNotNull-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlaceables$1":I
    move/from16 v25, v4

    move-object v4, v6

    check-cast v4, Landroidx/compose/ui/unit/Density;

    move-object/from16 v26, v11

    .end local v11    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .local v26, "$this$fastForEach$iv$iv":Ljava/util/List;
    invoke-interface {v6}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v11

    invoke-interface {v5, v4, v11}, Landroidx/compose/foundation/layout/WindowInsets;->getLeft(Landroidx/compose/ui/unit/Density;Landroidx/compose/ui/unit/LayoutDirection;)I

    move-result v11

    .line 384
    nop

    .line 387
    .local v11, "leftInset":I
    move/from16 v27, v12

    .end local v12    # "$i$f$fastForEach":I
    .local v27, "$i$f$fastForEach":I
    invoke-interface {v6}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v12

    invoke-interface {v5, v4, v12}, Landroidx/compose/foundation/layout/WindowInsets;->getRight(Landroidx/compose/ui/unit/Density;Landroidx/compose/ui/unit/LayoutDirection;)I

    move-result v12

    .line 386
    nop

    .line 388
    .local v12, "rightInset":I
    invoke-interface {v5, v4}, Landroidx/compose/foundation/layout/WindowInsets;->getBottom(Landroidx/compose/ui/unit/Density;)I

    move-result v4

    .line 389
    .local v4, "bottomInset":I
    nop

    .line 390
    nop

    .line 391
    move-object/from16 v28, v5

    neg-int v5, v11

    sub-int/2addr v5, v12

    .line 392
    move-object/from16 v29, v6

    neg-int v6, v4

    .line 390
    invoke-static {v7, v8, v5, v6}, Landroidx/compose/ui/unit/ConstraintsKt;->offset-NN6Ew-U(JII)J

    move-result-wide v5

    .line 389
    invoke-interface {v9, v5, v6}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v5

    .line 395
    move-object v6, v5

    .line 668
    .local v6, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v30, 0x0

    .line 395
    .local v30, "$i$a$-takeIf-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlaceables$1$1":I
    invoke-virtual {v6}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v31

    if-eqz v31, :cond_e

    invoke-virtual {v6}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v31

    if-eqz v31, :cond_e

    const/16 v23, 0x1

    goto :goto_c

    :cond_e
    const/16 v23, 0x0

    .end local v6    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v30    # "$i$a$-takeIf-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlaceables$1$1":I
    :goto_c
    if-eqz v23, :cond_f

    goto :goto_d

    :cond_f
    const/4 v5, 0x0

    .line 667
    .end local v4    # "bottomInset":I
    .end local v9    # "measurable":Landroidx/compose/ui/layout/Measurable;
    .end local v11    # "leftInset":I
    .end local v12    # "rightInset":I
    .end local v24    # "$i$a$-fastMapNotNull-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlaceables$1":I
    :goto_d
    if-eqz v5, :cond_10

    move-object v4, v5

    .line 669
    .local v4, "it$iv":Ljava/lang/Object;
    const/4 v5, 0x0

    .line 667
    .local v5, "$i$a$-let-ListUtilsKt$fastMapNotNull$2$1$iv":I
    move-object v6, v10

    check-cast v6, Ljava/util/Collection;

    invoke-interface {v6, v4}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 670
    .end local v4    # "it$iv":Ljava/lang/Object;
    .end local v5    # "$i$a$-let-ListUtilsKt$fastMapNotNull$2$1$iv":I
    :cond_10
    nop

    .line 666
    .end local v21    # "e$iv":Ljava/lang/Object;
    .end local v22    # "$i$a$-fastForEach-ListUtilsKt$fastMapNotNull$2$iv":I
    nop

    .line 664
    .end local v18    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v15, v15, 0x1

    move/from16 v9, v17

    move/from16 v4, v25

    move-object/from16 v11, v26

    move/from16 v12, v27

    move-object/from16 v5, v28

    move-object/from16 v6, v29

    goto :goto_b

    .end local v26    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v27    # "$i$f$fastForEach":I
    .local v11, "$this$fastForEach$iv$iv":Ljava/util/List;
    .local v12, "$i$f$fastForEach":I
    :cond_11
    move-object/from16 v26, v11

    move/from16 v27, v12

    .line 671
    .end local v11    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v12    # "$i$f$fastForEach":I
    .end local v15    # "index$iv$iv":I
    .restart local v26    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .restart local v27    # "$i$f$fastForEach":I
    nop

    .line 672
    .end local v26    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v27    # "$i$f$fastForEach":I
    move-object v4, v10

    check-cast v4, Ljava/util/List;

    .line 382
    .end local v10    # "target$iv":Ljava/util/ArrayList;
    .end local v16    # "$this$fastMapNotNull$iv":Ljava/util/List;
    .end local v17    # "$i$f$fastMapNotNull":I
    nop

    .line 381
    nop

    .line 398
    .local v4, "fabPlaceables":Ljava/util/List;
    move-object v5, v4

    check-cast v5, Ljava/util/Collection;

    invoke-interface {v5}, Ljava/util/Collection;->isEmpty()Z

    move-result v5

    const/4 v6, 0x1

    xor-int/2addr v5, v6

    if-eqz v5, :cond_1c

    .line 399
    move-object v5, v4

    .local v5, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v6, 0x0

    .line 673
    .local v6, "$i$f$fastMaxBy":I
    nop

    .line 674
    invoke-interface {v5}, Ljava/util/List;->isEmpty()Z

    move-result v7

    if-eqz v7, :cond_12

    const/4 v8, 0x0

    goto :goto_f

    .line 675
    :cond_12
    const/4 v7, 0x0

    invoke-interface {v5, v7}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v8

    .line 676
    .local v8, "maxElem$iv":Ljava/lang/Object;
    move-object v7, v8

    check-cast v7, Landroidx/compose/ui/layout/Placeable;

    .local v7, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v9, 0x0

    .line 399
    .local v9, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabWidth$1":I
    invoke-virtual {v7}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v7

    .line 676
    .end local v7    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v9    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabWidth$1":I
    nop

    .line 677
    .local v7, "maxValue$iv":I
    const/4 v9, 0x1

    .local v9, "i$iv":I
    invoke-static {v5}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v10

    if-gt v9, v10, :cond_14

    .line 678
    :goto_e
    invoke-interface {v5, v9}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v11

    .line 679
    .local v11, "e$iv":Ljava/lang/Object;
    move-object v12, v11

    check-cast v12, Landroidx/compose/ui/layout/Placeable;

    .local v12, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v15, 0x0

    .line 399
    .local v15, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabWidth$1":I
    invoke-virtual {v12}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v12

    .line 679
    .end local v12    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v15    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabWidth$1":I
    nop

    .line 680
    .local v12, "v$iv":I
    if-ge v7, v12, :cond_13

    .line 681
    move-object v8, v11

    .line 682
    move v7, v12

    .line 677
    .end local v11    # "e$iv":Ljava/lang/Object;
    .end local v12    # "v$iv":I
    :cond_13
    if-eq v9, v10, :cond_14

    add-int/lit8 v9, v9, 0x1

    goto :goto_e

    .line 685
    .end local v9    # "i$iv":I
    :cond_14
    nop

    .end local v5    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v6    # "$i$f$fastMaxBy":I
    .end local v7    # "maxValue$iv":I
    .end local v8    # "maxElem$iv":Ljava/lang/Object;
    :goto_f
    invoke-static {v8}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;)V

    check-cast v8, Landroidx/compose/ui/layout/Placeable;

    .line 399
    invoke-virtual {v8}, Landroidx/compose/ui/layout/Placeable;->getWidth()I

    move-result v5

    .line 400
    .local v5, "fabWidth":I
    move-object v6, v4

    .local v6, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 686
    .local v7, "$i$f$fastMaxBy":I
    nop

    .line 687
    invoke-interface {v6}, Ljava/util/List;->isEmpty()Z

    move-result v8

    if-eqz v8, :cond_15

    const/4 v9, 0x0

    goto :goto_11

    .line 688
    :cond_15
    const/4 v8, 0x0

    invoke-interface {v6, v8}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v9

    .line 689
    .local v9, "maxElem$iv":Ljava/lang/Object;
    move-object v8, v9

    check-cast v8, Landroidx/compose/ui/layout/Placeable;

    .local v8, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v10, 0x0

    .line 400
    .local v10, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabHeight$1":I
    invoke-virtual {v8}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v8

    .line 689
    .end local v8    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v10    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabHeight$1":I
    nop

    .line 690
    .local v8, "maxValue$iv":I
    const/4 v10, 0x1

    .local v10, "i$iv":I
    invoke-static {v6}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v11

    if-gt v10, v11, :cond_17

    .line 691
    :goto_10
    invoke-interface {v6, v10}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v12

    .line 692
    .local v12, "e$iv":Ljava/lang/Object;
    move-object v15, v12

    check-cast v15, Landroidx/compose/ui/layout/Placeable;

    .local v15, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v16, 0x0

    .line 400
    .local v16, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabHeight$1":I
    invoke-virtual {v15}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v15

    .line 692
    .end local v15    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v16    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabPlacement$fabHeight$1":I
    nop

    .line 693
    .local v15, "v$iv":I
    if-ge v8, v15, :cond_16

    .line 694
    move-object v9, v12

    .line 695
    move v8, v15

    .line 690
    .end local v12    # "e$iv":Ljava/lang/Object;
    .end local v15    # "v$iv":I
    :cond_16
    if-eq v10, v11, :cond_17

    add-int/lit8 v10, v10, 0x1

    goto :goto_10

    .line 698
    .end local v10    # "i$iv":I
    :cond_17
    nop

    .end local v6    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v7    # "$i$f$fastMaxBy":I
    .end local v8    # "maxValue$iv":I
    .end local v9    # "maxElem$iv":Ljava/lang/Object;
    :goto_11
    invoke-static {v9}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;)V

    check-cast v9, Landroidx/compose/ui/layout/Placeable;

    .line 400
    invoke-virtual {v9}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v6

    .line 402
    .local v6, "fabHeight":I
    iget v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$fabPosition:I

    .line 403
    sget-object v8, Landroidx/compose/material3/FabPosition;->Companion:Landroidx/compose/material3/FabPosition$Companion;

    invoke-virtual {v8}, Landroidx/compose/material3/FabPosition$Companion;->getStart-ERTFSPs()I

    move-result v8

    invoke-static {v7, v8}, Landroidx/compose/material3/FabPosition;->equals-impl0(II)Z

    move-result v8

    if-eqz v8, :cond_19

    .line 404
    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    invoke-interface {v7}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v7

    sget-object v8, Landroidx/compose/ui/unit/LayoutDirection;->Ltr:Landroidx/compose/ui/unit/LayoutDirection;

    if-ne v7, v8, :cond_18

    .line 405
    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    invoke-static {}, Landroidx/compose/material3/ScaffoldKt;->access$getFabSpacing$p()F

    move-result v8

    invoke-interface {v7, v8}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->roundToPx-0680j_4(F)I

    move-result v7

    goto :goto_12

    .line 407
    :cond_18
    iget v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutWidth:I

    iget-object v8, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    invoke-static {}, Landroidx/compose/material3/ScaffoldKt;->access$getFabSpacing$p()F

    move-result v9

    invoke-interface {v8, v9}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->roundToPx-0680j_4(F)I

    move-result v8

    sub-int/2addr v7, v8

    sub-int/2addr v7, v5

    goto :goto_12

    .line 410
    :cond_19
    sget-object v8, Landroidx/compose/material3/FabPosition;->Companion:Landroidx/compose/material3/FabPosition$Companion;

    invoke-virtual {v8}, Landroidx/compose/material3/FabPosition$Companion;->getEnd-ERTFSPs()I

    move-result v8

    invoke-static {v7, v8}, Landroidx/compose/material3/FabPosition;->equals-impl0(II)Z

    move-result v7

    if-eqz v7, :cond_1b

    .line 411
    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    invoke-interface {v7}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v7

    sget-object v8, Landroidx/compose/ui/unit/LayoutDirection;->Ltr:Landroidx/compose/ui/unit/LayoutDirection;

    if-ne v7, v8, :cond_1a

    .line 412
    iget v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutWidth:I

    iget-object v8, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    invoke-static {}, Landroidx/compose/material3/ScaffoldKt;->access$getFabSpacing$p()F

    move-result v9

    invoke-interface {v8, v9}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->roundToPx-0680j_4(F)I

    move-result v8

    sub-int/2addr v7, v8

    sub-int/2addr v7, v5

    goto :goto_12

    .line 414
    :cond_1a
    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    invoke-static {}, Landroidx/compose/material3/ScaffoldKt;->access$getFabSpacing$p()F

    move-result v8

    invoke-interface {v7, v8}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->roundToPx-0680j_4(F)I

    move-result v7

    goto :goto_12

    .line 417
    :cond_1b
    iget v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutWidth:I

    sub-int/2addr v7, v5

    div-int/lit8 v7, v7, 0x2

    .line 402
    :goto_12
    nop

    .line 420
    .local v7, "fabLeftOffset":I
    new-instance v8, Landroidx/compose/material3/FabPlacement;

    .line 421
    nop

    .line 422
    nop

    .line 423
    nop

    .line 420
    invoke-direct {v8, v7, v5, v6}, Landroidx/compose/material3/FabPlacement;-><init>(III)V

    .end local v5    # "fabWidth":I
    .end local v6    # "fabHeight":I
    .end local v7    # "fabLeftOffset":I
    goto :goto_13

    .line 426
    :cond_1c
    const/4 v8, 0x0

    .line 398
    :goto_13
    move-object v5, v8

    .line 429
    .local v5, "fabPlacement":Landroidx/compose/material3/FabPlacement;
    iget-object v6, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    sget-object v7, Landroidx/compose/material3/ScaffoldLayoutContent;->BottomBar:Landroidx/compose/material3/ScaffoldLayoutContent;

    new-instance v8, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarPlaceables$1;

    iget-object v9, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$bottomBar:Lkotlin/jvm/functions/Function2;

    invoke-direct {v8, v5, v9}, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarPlaceables$1;-><init>(Landroidx/compose/material3/FabPlacement;Lkotlin/jvm/functions/Function2;)V

    const v9, -0x2f274393

    const/4 v10, 0x1

    invoke-static {v9, v10, v8}, Landroidx/compose/runtime/internal/ComposableLambdaKt;->composableLambdaInstance(IZLjava/lang/Object;)Landroidx/compose/runtime/internal/ComposableLambda;

    move-result-object v8

    check-cast v8, Lkotlin/jvm/functions/Function2;

    invoke-interface {v6, v7, v8}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v6

    .line 434
    iget-wide v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$looseConstraints:J

    .local v6, "$this$fastMap$iv":Ljava/util/List;
    const/4 v9, 0x0

    .line 699
    .local v9, "$i$f$fastMap":I
    nop

    .line 700
    new-instance v10, Ljava/util/ArrayList;

    invoke-interface {v6}, Ljava/util/List;->size()I

    move-result v11

    invoke-direct {v10, v11}, Ljava/util/ArrayList;-><init>(I)V

    .line 701
    .local v10, "target$iv":Ljava/util/ArrayList;
    move-object v11, v6

    .local v11, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v12, 0x0

    .line 702
    .local v12, "$i$f$fastForEach":I
    nop

    .line 703
    const/4 v15, 0x0

    move-object/from16 v16, v6

    .end local v6    # "$this$fastMap$iv":Ljava/util/List;
    .local v15, "index$iv$iv":I
    .local v16, "$this$fastMap$iv":Ljava/util/List;
    invoke-interface {v11}, Ljava/util/List;->size()I

    move-result v6

    :goto_14
    if-ge v15, v6, :cond_1d

    .line 704
    invoke-interface {v11, v15}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v17

    .line 705
    .local v17, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v18, v17

    .local v18, "it$iv":Ljava/lang/Object;
    const/16 v21, 0x0

    .line 706
    .local v21, "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move/from16 v22, v6

    move-object v6, v10

    check-cast v6, Ljava/util/Collection;

    move/from16 v24, v9

    .end local v9    # "$i$f$fastMap":I
    .local v24, "$i$f$fastMap":I
    move-object/from16 v9, v18

    check-cast v9, Landroidx/compose/ui/layout/Measurable;

    .local v9, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v25, 0x0

    .line 434
    .local v25, "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarPlaceables$2":I
    invoke-interface {v9, v7, v8}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v9

    .line 706
    .end local v9    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v25    # "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarPlaceables$2":I
    invoke-interface {v6, v9}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 707
    nop

    .line 705
    .end local v18    # "it$iv":Ljava/lang/Object;
    .end local v21    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 703
    .end local v17    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v15, v15, 0x1

    move/from16 v6, v22

    move/from16 v9, v24

    goto :goto_14

    .end local v24    # "$i$f$fastMap":I
    .local v9, "$i$f$fastMap":I
    :cond_1d
    move/from16 v24, v9

    .line 708
    .end local v9    # "$i$f$fastMap":I
    .end local v15    # "index$iv$iv":I
    .restart local v24    # "$i$f$fastMap":I
    nop

    .line 709
    .end local v11    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v12    # "$i$f$fastForEach":I
    move-object v15, v10

    check-cast v15, Ljava/util/List;

    .line 434
    .end local v10    # "target$iv":Ljava/util/ArrayList;
    .end local v16    # "$this$fastMap$iv":Ljava/util/List;
    .end local v24    # "$i$f$fastMap":I
    nop

    .line 429
    nop

    .line 436
    .local v15, "bottomBarPlaceables":Ljava/util/List;
    move-object v6, v15

    .local v6, "$this$fastMaxBy$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 710
    .local v7, "$i$f$fastMaxBy":I
    nop

    .line 711
    invoke-interface {v6}, Ljava/util/List;->isEmpty()Z

    move-result v8

    if-eqz v8, :cond_1e

    const/4 v9, 0x0

    goto :goto_16

    .line 712
    :cond_1e
    const/4 v8, 0x0

    invoke-interface {v6, v8}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v9

    .line 713
    .local v9, "maxElem$iv":Ljava/lang/Object;
    move-object v10, v9

    check-cast v10, Landroidx/compose/ui/layout/Placeable;

    .local v10, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v11, 0x0

    .line 436
    .local v11, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarHeight$1":I
    invoke-virtual {v10}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v10

    .line 713
    .end local v10    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v11    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarHeight$1":I
    nop

    .line 714
    .local v10, "maxValue$iv":I
    const/4 v11, 0x1

    .local v11, "i$iv":I
    invoke-static {v6}, Lkotlin/collections/CollectionsKt;->getLastIndex(Ljava/util/List;)I

    move-result v12

    if-gt v11, v12, :cond_20

    .line 715
    :goto_15
    invoke-interface {v6, v11}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v16

    .line 716
    .local v16, "e$iv":Ljava/lang/Object;
    move-object/from16 v17, v16

    check-cast v17, Landroidx/compose/ui/layout/Placeable;

    .local v17, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v18, 0x0

    .line 436
    .local v18, "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarHeight$1":I
    invoke-virtual/range {v17 .. v17}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v17

    .line 716
    .end local v17    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v18    # "$i$a$-fastMaxBy-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bottomBarHeight$1":I
    move/from16 v18, v17

    .line 717
    .local v18, "v$iv":I
    move/from16 v8, v18

    .end local v18    # "v$iv":I
    .local v8, "v$iv":I
    if-ge v10, v8, :cond_1f

    .line 718
    move-object/from16 v9, v16

    .line 719
    move v10, v8

    .line 714
    .end local v8    # "v$iv":I
    .end local v16    # "e$iv":Ljava/lang/Object;
    :cond_1f
    if-eq v11, v12, :cond_20

    add-int/lit8 v11, v11, 0x1

    const/4 v8, 0x0

    goto :goto_15

    .line 722
    .end local v11    # "i$iv":I
    :cond_20
    nop

    .line 436
    .end local v6    # "$this$fastMaxBy$iv":Ljava/util/List;
    .end local v7    # "$i$f$fastMaxBy":I
    .end local v9    # "maxElem$iv":Ljava/lang/Object;
    .end local v10    # "maxValue$iv":I
    :goto_16
    check-cast v9, Landroidx/compose/ui/layout/Placeable;

    if-eqz v9, :cond_21

    invoke-virtual {v9}, Landroidx/compose/ui/layout/Placeable;->getHeight()I

    move-result v6

    invoke-static {v6}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    goto :goto_17

    :cond_21
    const/4 v6, 0x0

    .line 437
    .local v6, "bottomBarHeight":Ljava/lang/Integer;
    :goto_17
    if-eqz v5, :cond_23

    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    iget-object v8, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    move-object v9, v5

    .local v9, "it":Landroidx/compose/material3/FabPlacement;
    const/4 v10, 0x0

    .line 438
    .local v10, "$i$a$-let-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabOffsetFromBottom$1":I
    if-nez v6, :cond_22

    .line 439
    invoke-virtual {v9}, Landroidx/compose/material3/FabPlacement;->getHeight()I

    move-result v11

    invoke-static {}, Landroidx/compose/material3/ScaffoldKt;->access$getFabSpacing$p()F

    move-result v12

    invoke-interface {v7, v12}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->roundToPx-0680j_4(F)I

    move-result v12

    add-int/2addr v11, v12

    .line 440
    check-cast v7, Landroidx/compose/ui/unit/Density;

    invoke-interface {v8, v7}, Landroidx/compose/foundation/layout/WindowInsets;->getBottom(Landroidx/compose/ui/unit/Density;)I

    move-result v7

    .line 439
    add-int/2addr v11, v7

    goto :goto_18

    .line 444
    :cond_22
    invoke-virtual {v6}, Ljava/lang/Integer;->intValue()I

    move-result v8

    invoke-virtual {v9}, Landroidx/compose/material3/FabPlacement;->getHeight()I

    move-result v11

    add-int/2addr v8, v11

    invoke-static {}, Landroidx/compose/material3/ScaffoldKt;->access$getFabSpacing$p()F

    move-result v11

    invoke-interface {v7, v11}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->roundToPx-0680j_4(F)I

    move-result v7

    add-int v11, v8, v7

    .line 438
    :goto_18
    nop

    .end local v9    # "it":Landroidx/compose/material3/FabPlacement;
    .end local v10    # "$i$a$-let-ScaffoldKt$LegacyScaffoldLayout$1$1$1$fabOffsetFromBottom$1":I
    invoke-static {v11}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v7

    .line 437
    move-object/from16 v19, v7

    goto :goto_19

    :cond_23
    const/16 v19, 0x0

    :goto_19
    move-object/from16 v7, v19

    .line 448
    .local v7, "fabOffsetFromBottom":Ljava/lang/Integer;
    if-eqz v2, :cond_26

    .line 449
    nop

    .line 450
    if-eqz v7, :cond_24

    invoke-virtual {v7}, Ljava/lang/Integer;->intValue()I

    move-result v8

    goto :goto_1a

    :cond_24
    if-eqz v6, :cond_25

    invoke-virtual {v6}, Ljava/lang/Integer;->intValue()I

    move-result v8

    goto :goto_1a

    .line 451
    :cond_25
    iget-object v8, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    iget-object v9, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    check-cast v9, Landroidx/compose/ui/unit/Density;

    invoke-interface {v8, v9}, Landroidx/compose/foundation/layout/WindowInsets;->getBottom(Landroidx/compose/ui/unit/Density;)I

    move-result v8

    .line 449
    :goto_1a
    add-int/2addr v8, v2

    goto :goto_1b

    .line 453
    :cond_26
    const/4 v8, 0x0

    .line 448
    :goto_1b
    nop

    .line 456
    .local v8, "snackbarOffsetFromBottom":I
    iget-object v9, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    sget-object v12, Landroidx/compose/material3/ScaffoldLayoutContent;->MainContent:Landroidx/compose/material3/ScaffoldLayoutContent;

    new-instance v11, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1$bodyContentPlaceables$1;

    iget-object v10, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    move/from16 v18, v2

    .end local v2    # "snackbarHeight":I
    .local v18, "snackbarHeight":I
    iget-object v2, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    move-object/from16 v19, v7

    .end local v7    # "fabOffsetFromBottom":Ljava/lang/Integer;
    .local v19, "fabOffsetFromBottom":Ljava/lang/Integer;
    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$content:Lkotlin/jvm/functions/Function3;

    move-object/from16 v16, v10

    move-object v10, v11

    move-object/from16 v21, v4

    move-object v4, v11

    .end local v4    # "fabPlaceables":Ljava/util/List;
    .local v21, "fabPlaceables":Ljava/util/List;
    move-object/from16 v11, v16

    move-object/from16 v22, v5

    move-object v5, v12

    .end local v5    # "fabPlacement":Landroidx/compose/material3/FabPlacement;
    .local v22, "fabPlacement":Landroidx/compose/material3/FabPlacement;
    move-object v12, v2

    move-object/from16 v16, v6

    move-object/from16 v17, v7

    invoke-direct/range {v10 .. v17}, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1$bodyContentPlaceables$1;-><init>(Landroidx/compose/foundation/layout/WindowInsets;Landroidx/compose/ui/layout/SubcomposeMeasureScope;Ljava/util/List;ILjava/util/List;Ljava/lang/Integer;Lkotlin/jvm/functions/Function3;)V

    const v2, 0x1d8622be

    const/4 v7, 0x1

    invoke-static {v2, v7, v4}, Landroidx/compose/runtime/internal/ComposableLambdaKt;->composableLambdaInstance(IZLjava/lang/Object;)Landroidx/compose/runtime/internal/ComposableLambda;

    move-result-object v2

    check-cast v2, Lkotlin/jvm/functions/Function2;

    invoke-interface {v9, v5, v2}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->subcompose(Ljava/lang/Object;Lkotlin/jvm/functions/Function2;)Ljava/util/List;

    move-result-object v2

    .line 475
    iget-wide v4, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$looseConstraints:J

    .local v2, "$this$fastMap$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 723
    .local v7, "$i$f$fastMap":I
    nop

    .line 724
    new-instance v9, Ljava/util/ArrayList;

    invoke-interface {v2}, Ljava/util/List;->size()I

    move-result v10

    invoke-direct {v9, v10}, Ljava/util/ArrayList;-><init>(I)V

    .line 725
    .local v9, "target$iv":Ljava/util/ArrayList;
    move-object v10, v2

    .local v10, "$this$fastForEach$iv$iv":Ljava/util/List;
    const/4 v11, 0x0

    .line 726
    .local v11, "$i$f$fastForEach":I
    nop

    .line 727
    const/4 v12, 0x0

    move-object/from16 v16, v2

    .end local v2    # "$this$fastMap$iv":Ljava/util/List;
    .local v12, "index$iv$iv":I
    .local v16, "$this$fastMap$iv":Ljava/util/List;
    invoke-interface {v10}, Ljava/util/List;->size()I

    move-result v2

    :goto_1c
    if-ge v12, v2, :cond_27

    .line 728
    invoke-interface {v10, v12}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v17

    .line 729
    .local v17, "item$iv$iv":Ljava/lang/Object;
    move-object/from16 v23, v17

    .local v23, "it$iv":Ljava/lang/Object;
    const/16 v24, 0x0

    .line 730
    .local v24, "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    move/from16 v25, v2

    move-object v2, v9

    check-cast v2, Ljava/util/Collection;

    move/from16 v26, v7

    .end local v7    # "$i$f$fastMap":I
    .local v26, "$i$f$fastMap":I
    move-object/from16 v7, v23

    check-cast v7, Landroidx/compose/ui/layout/Measurable;

    .local v7, "it":Landroidx/compose/ui/layout/Measurable;
    const/16 v27, 0x0

    .line 475
    .local v27, "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bodyContentPlaceables$2":I
    invoke-interface {v7, v4, v5}, Landroidx/compose/ui/layout/Measurable;->measure-BRTryo0(J)Landroidx/compose/ui/layout/Placeable;

    move-result-object v7

    .line 730
    .end local v7    # "it":Landroidx/compose/ui/layout/Measurable;
    .end local v27    # "$i$a$-fastMap-ScaffoldKt$LegacyScaffoldLayout$1$1$1$bodyContentPlaceables$2":I
    invoke-interface {v2, v7}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 731
    nop

    .line 729
    .end local v23    # "it$iv":Ljava/lang/Object;
    .end local v24    # "$i$a$-fastForEach-ListUtilsKt$fastMap$2$iv":I
    nop

    .line 727
    .end local v17    # "item$iv$iv":Ljava/lang/Object;
    add-int/lit8 v12, v12, 0x1

    move/from16 v2, v25

    move/from16 v7, v26

    goto :goto_1c

    .end local v26    # "$i$f$fastMap":I
    .local v7, "$i$f$fastMap":I
    :cond_27
    move/from16 v26, v7

    .line 732
    .end local v7    # "$i$f$fastMap":I
    .end local v12    # "index$iv$iv":I
    .restart local v26    # "$i$f$fastMap":I
    nop

    .line 733
    .end local v10    # "$this$fastForEach$iv$iv":Ljava/util/List;
    .end local v11    # "$i$f$fastForEach":I
    move-object v2, v9

    check-cast v2, Ljava/util/List;

    .line 475
    .end local v9    # "target$iv":Ljava/util/ArrayList;
    .end local v16    # "$this$fastMap$iv":Ljava/util/List;
    .end local v26    # "$i$f$fastMap":I
    nop

    .line 456
    nop

    .line 478
    .local v2, "bodyContentPlaceables":Ljava/util/List;
    move-object v4, v2

    .local v4, "$this$fastForEach$iv":Ljava/util/List;
    const/4 v5, 0x0

    .line 734
    .local v5, "$i$f$fastForEach":I
    nop

    .line 735
    const/4 v7, 0x0

    .local v7, "index$iv":I
    invoke-interface {v4}, Ljava/util/List;->size()I

    move-result v9

    :goto_1d
    if-ge v7, v9, :cond_28

    .line 736
    invoke-interface {v4, v7}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v10

    .line 737
    .local v10, "item$iv":Ljava/lang/Object;
    move-object v11, v10

    check-cast v11, Landroidx/compose/ui/layout/Placeable;

    .local v11, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v12, 0x0

    .line 479
    .local v12, "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$1":I
    const/16 v25, 0x0

    const/16 v26, 0x0

    const/16 v27, 0x0

    const/16 v28, 0x4

    const/16 v29, 0x0

    move-object/from16 v23, p1

    move-object/from16 v24, v11

    invoke-static/range {v23 .. v29}, Landroidx/compose/ui/layout/Placeable$PlacementScope;->place$default(Landroidx/compose/ui/layout/Placeable$PlacementScope;Landroidx/compose/ui/layout/Placeable;IIFILjava/lang/Object;)V

    .line 480
    nop

    .line 737
    .end local v11    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v12    # "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$1":I
    nop

    .line 735
    .end local v10    # "item$iv":Ljava/lang/Object;
    add-int/lit8 v7, v7, 0x1

    goto :goto_1d

    .line 739
    .end local v7    # "index$iv":I
    :cond_28
    nop

    .line 481
    .end local v4    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v5    # "$i$f$fastForEach":I
    move-object v4, v13

    .restart local v4    # "$this$fastForEach$iv":Ljava/util/List;
    const/4 v5, 0x0

    .line 740
    .restart local v5    # "$i$f$fastForEach":I
    nop

    .line 741
    const/4 v7, 0x0

    .restart local v7    # "index$iv":I
    invoke-interface {v4}, Ljava/util/List;->size()I

    move-result v9

    :goto_1e
    if-ge v7, v9, :cond_29

    .line 742
    invoke-interface {v4, v7}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v10

    .line 743
    .restart local v10    # "item$iv":Ljava/lang/Object;
    move-object v11, v10

    check-cast v11, Landroidx/compose/ui/layout/Placeable;

    .restart local v11    # "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v12, 0x0

    .line 482
    .local v12, "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$2":I
    const/16 v25, 0x0

    const/16 v26, 0x0

    const/16 v27, 0x0

    const/16 v28, 0x4

    const/16 v29, 0x0

    move-object/from16 v23, p1

    move-object/from16 v24, v11

    invoke-static/range {v23 .. v29}, Landroidx/compose/ui/layout/Placeable$PlacementScope;->place$default(Landroidx/compose/ui/layout/Placeable$PlacementScope;Landroidx/compose/ui/layout/Placeable;IIFILjava/lang/Object;)V

    .line 483
    nop

    .line 743
    .end local v11    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v12    # "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$2":I
    nop

    .line 741
    .end local v10    # "item$iv":Ljava/lang/Object;
    add-int/lit8 v7, v7, 0x1

    goto :goto_1e

    .line 745
    .end local v7    # "index$iv":I
    :cond_29
    nop

    .line 484
    .end local v4    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v5    # "$i$f$fastForEach":I
    iget v4, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutWidth:I

    iget-object v5, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$contentWindowInsets:Landroidx/compose/foundation/layout/WindowInsets;

    iget-object v7, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$this_SubcomposeLayout:Landroidx/compose/ui/layout/SubcomposeMeasureScope;

    iget v9, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutHeight:I

    move-object v10, v1

    .local v10, "$this$fastForEach$iv":Ljava/util/List;
    const/4 v11, 0x0

    .line 746
    .local v11, "$i$f$fastForEach":I
    nop

    .line 747
    const/4 v12, 0x0

    move-object/from16 v16, v1

    .end local v1    # "snackbarPlaceables":Ljava/util/List;
    .local v12, "index$iv":I
    .local v16, "snackbarPlaceables":Ljava/util/List;
    invoke-interface {v10}, Ljava/util/List;->size()I

    move-result v1

    :goto_1f
    if-ge v12, v1, :cond_2a

    .line 748
    invoke-interface {v10, v12}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v17

    .line 749
    .local v17, "item$iv":Ljava/lang/Object;
    move-object/from16 v30, v17

    check-cast v30, Landroidx/compose/ui/layout/Placeable;

    .local v30, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v31, 0x0

    .line 485
    .local v31, "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$3":I
    nop

    .line 486
    sub-int v23, v4, v3

    div-int/lit8 v23, v23, 0x2

    .line 487
    move/from16 v32, v1

    move-object v1, v7

    check-cast v1, Landroidx/compose/ui/unit/Density;

    move-object/from16 v33, v2

    .end local v2    # "bodyContentPlaceables":Ljava/util/List;
    .local v33, "bodyContentPlaceables":Ljava/util/List;
    invoke-interface {v7}, Landroidx/compose/ui/layout/SubcomposeMeasureScope;->getLayoutDirection()Landroidx/compose/ui/unit/LayoutDirection;

    move-result-object v2

    invoke-interface {v5, v1, v2}, Landroidx/compose/foundation/layout/WindowInsets;->getLeft(Landroidx/compose/ui/unit/Density;Landroidx/compose/ui/unit/LayoutDirection;)I

    move-result v1

    .line 486
    add-int v25, v23, v1

    .line 488
    sub-int v26, v9, v8

    .line 485
    const/16 v27, 0x0

    const/16 v28, 0x4

    const/16 v29, 0x0

    move-object/from16 v23, p1

    move-object/from16 v24, v30

    invoke-static/range {v23 .. v29}, Landroidx/compose/ui/layout/Placeable$PlacementScope;->place$default(Landroidx/compose/ui/layout/Placeable$PlacementScope;Landroidx/compose/ui/layout/Placeable;IIFILjava/lang/Object;)V

    .line 490
    nop

    .line 749
    .end local v30    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v31    # "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$3":I
    nop

    .line 747
    .end local v17    # "item$iv":Ljava/lang/Object;
    add-int/lit8 v12, v12, 0x1

    move/from16 v1, v32

    move-object/from16 v2, v33

    goto :goto_1f

    .end local v33    # "bodyContentPlaceables":Ljava/util/List;
    .restart local v2    # "bodyContentPlaceables":Ljava/util/List;
    :cond_2a
    move-object/from16 v33, v2

    .line 751
    .end local v2    # "bodyContentPlaceables":Ljava/util/List;
    .end local v12    # "index$iv":I
    .restart local v33    # "bodyContentPlaceables":Ljava/util/List;
    nop

    .line 492
    .end local v10    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v11    # "$i$f$fastForEach":I
    iget v1, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutHeight:I

    move-object v2, v15

    .local v2, "$this$fastForEach$iv":Ljava/util/List;
    const/4 v4, 0x0

    .line 752
    .local v4, "$i$f$fastForEach":I
    nop

    .line 753
    const/4 v5, 0x0

    .local v5, "index$iv":I
    invoke-interface {v2}, Ljava/util/List;->size()I

    move-result v7

    :goto_20
    if-ge v5, v7, :cond_2c

    .line 754
    invoke-interface {v2, v5}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v9

    .line 755
    .local v9, "item$iv":Ljava/lang/Object;
    move-object v10, v9

    check-cast v10, Landroidx/compose/ui/layout/Placeable;

    .local v10, "it":Landroidx/compose/ui/layout/Placeable;
    const/4 v11, 0x0

    .line 493
    .local v11, "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$4":I
    const/16 v25, 0x0

    if-eqz v6, :cond_2b

    invoke-virtual {v6}, Ljava/lang/Integer;->intValue()I

    move-result v12

    goto :goto_21

    :cond_2b
    const/4 v12, 0x0

    :goto_21
    sub-int v26, v1, v12

    const/16 v27, 0x0

    const/16 v28, 0x4

    const/16 v29, 0x0

    move-object/from16 v23, p1

    move-object/from16 v24, v10

    invoke-static/range {v23 .. v29}, Landroidx/compose/ui/layout/Placeable$PlacementScope;->place$default(Landroidx/compose/ui/layout/Placeable$PlacementScope;Landroidx/compose/ui/layout/Placeable;IIFILjava/lang/Object;)V

    .line 494
    nop

    .line 755
    .end local v10    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v11    # "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$4":I
    nop

    .line 753
    .end local v9    # "item$iv":Ljava/lang/Object;
    add-int/lit8 v5, v5, 0x1

    goto :goto_20

    .line 757
    .end local v5    # "index$iv":I
    :cond_2c
    nop

    .line 496
    .end local v2    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v4    # "$i$f$fastForEach":I
    if-eqz v22, :cond_2e

    iget v1, v0, Landroidx/compose/material3/ScaffoldKt$LegacyScaffoldLayout$1$1$1;->$layoutHeight:I

    move-object/from16 v2, v22

    .local v2, "placement":Landroidx/compose/material3/FabPlacement;
    const/4 v4, 0x0

    .line 497
    .local v4, "$i$a$-let-ScaffoldKt$LegacyScaffoldLayout$1$1$1$5":I
    move-object/from16 v5, v21

    .local v5, "$this$fastForEach$iv":Ljava/util/List;
    const/4 v7, 0x0

    .line 758
    .local v7, "$i$f$fastForEach":I
    nop

    .line 759
    const/4 v9, 0x0

    .local v9, "index$iv":I
    invoke-interface {v5}, Ljava/util/List;->size()I

    move-result v10

    :goto_22
    if-ge v9, v10, :cond_2d

    .line 760
    invoke-interface {v5, v9}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v11

    .line 761
    .local v11, "item$iv":Ljava/lang/Object;
    move-object v12, v11

    check-cast v12, Landroidx/compose/ui/layout/Placeable;

    .local v12, "it":Landroidx/compose/ui/layout/Placeable;
    const/16 v17, 0x0

    .line 498
    .local v17, "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$5$1":I
    invoke-virtual {v2}, Landroidx/compose/material3/FabPlacement;->getLeft()I

    move-result v25

    invoke-static/range {v19 .. v19}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;)V

    invoke-virtual/range {v19 .. v19}, Ljava/lang/Integer;->intValue()I

    move-result v20

    sub-int v26, v1, v20

    const/16 v27, 0x0

    const/16 v28, 0x4

    const/16 v29, 0x0

    move-object/from16 v23, p1

    move-object/from16 v24, v12

    invoke-static/range {v23 .. v29}, Landroidx/compose/ui/layout/Placeable$PlacementScope;->place$default(Landroidx/compose/ui/layout/Placeable$PlacementScope;Landroidx/compose/ui/layout/Placeable;IIFILjava/lang/Object;)V

    .line 499
    nop

    .line 761
    .end local v12    # "it":Landroidx/compose/ui/layout/Placeable;
    .end local v17    # "$i$a$-fastForEach-ScaffoldKt$LegacyScaffoldLayout$1$1$1$5$1":I
    nop

    .line 759
    .end local v11    # "item$iv":Ljava/lang/Object;
    add-int/lit8 v9, v9, 0x1

    goto :goto_22

    .line 763
    .end local v9    # "index$iv":I
    :cond_2d
    nop

    .line 500
    .end local v5    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v7    # "$i$f$fastForEach":I
    nop

    .end local v2    # "placement":Landroidx/compose/material3/FabPlacement;
    .end local v4    # "$i$a$-let-ScaffoldKt$LegacyScaffoldLayout$1$1$1$5":I
    sget-object v1, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 496
    sget-object v1, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 501
    :cond_2e
    return-void
.end method
