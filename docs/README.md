# Docs Map

## Active Working Docs

- `../PROJECT_CONTEXT_HANDOFF.md`
- `../READER_SETTINGS_SNAPSHOT_2026-03-25.md`
- `../TASKLIST_00_MASTER_STRUCTURE.md`
- `../TASKLIST_01_READER_EXPERIENCE.md`
- `../TASKLIST_02_LIBRARY_GAMIFICATION.md`
- `../TASKLIST_03_TRANSLATION_AI_TTS.md`
- `../TASKLIST_04_SETTINGS_IA_LOCALIZATION.md`
- `../TASKLIST_05_PLATFORM_FOUNDATION.md`
- `active/SETTINGS_CAPABILITY_MAP.md`
- `active/LOCALIZATION_AUDIT.md`
- `active/QA_REGRESSION_CHECKLIST.md`
- `active/TRANSLATION_MODULE_TZ.md`
- `active/THIRD_PARTY_DICTIONARIES.md`
- `active/DJVU_RENDERER_RESEARCH.md`

## Reference Docs

- `reference/DOCUMENT_RETENTION_AUDIT.md`
- `reference/CUSTOMIZATION_LIBRARY_STRUCTURE_BACKUP.md`
- `reference/CLAUDE.md`
- `reference/Геймификация приложения для чтения книг и комиксов с маскотом.md`
- `reference/Mr.Comic_gameficator.txt`
- `reference/books/Packt.Mastering.Kotlin.for.Android.14.1837631719.pdf`

## Archive

- `archive/roadmaps/`
- `archive/checkpoints/`
