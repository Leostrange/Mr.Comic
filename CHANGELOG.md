...
- Стабилизирована полезная область текстовой страницы: показ и скрытие chrome-панелей больше не должны обрезать верхние и нижние строки.
- Исправлены вертикальные отступы, обработка последней строки, адаптация крупных титульных заголовков и разрывы заголовков глав.
- Улучшены HTML/Markdown reflow, сноски и защита от случайного выделения текста во время перелистывания.
- В графическом ридере оставлены профильные настройки и восстановлены пресеты День, Сепия и Ночь.

### Library and themes

- Возвращены контрастные белые подложки формата и прогресса на обложках.
- Удалён дублирующий конструктор тем.
- Пресет темы записывается атомарно; предпросмотр обновляется согласованно.

### Dictionaries

- Добавлены импорт, экспорт, массовый экспорт, удаление и компактный экран управления.
- Поддержан импорт одиночного `.dbpack`, gzip-данных и архива модулей.
- Десять словарных баз вынесены из APK в независимый выпуск `dictionary-modules-v1.0.0`.

### Build and architecture

- Исправлены модели и калькулятор прогресса геймификации.
- Версия приложения обновлена до 2.4.0 (`versionCode 4`).
- Release workflow использует подготовленные release notes и больше не перезаписывает журнал изменений.
